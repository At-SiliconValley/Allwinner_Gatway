...
static void App_Application_Init(void){
    ...
    //1、初始化MQTT
    ComStatus status = Driver_MQTT_Init(App_Application_MQTTReceiveHandle);
    if( status == COM_FAIL ){
        log_info("mqtt 初始化失败");
        return;
    }
    ...
    status = Common_Poll_AddTask(&upTask );
    if(status == COM_FAIL){
        log_info("上行任务添加失败");
        Driver_Modbus_Destory();
        Driver_MQTT_Deinit();
    ...
    status = Common_Poll_AddTask(&downTask );
    if(status == COM_FAIL){
        log_info("下行任务添加失败");
        Driver_Modbus_Destory();
        Driver_MQTT_Deinit();
