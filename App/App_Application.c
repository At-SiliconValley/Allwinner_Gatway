 ...
 *   5、添加任务到线程池：
 *      a. 添加上行任务：
 *         - 定义 Task upTask = { .args = NULL, .Func = App_Application_UpTaskFunc }
 ...
 *              调用 Driver_Modbus_WriteHoldRegisters(id->valueint, TARGET_SPEED_ADDR, 2, fx.arr)
 *            - 发送目标角度：
 *              fx.data = (float)targetAngle->valuedouble
 *              调用 Driver_Modbus_WriteHoldRegisters(id->valueint, TARGET_ANGLE_ADDR, 2, fx.arr)
 *            - 启动/停止电机：
 *              调用 Driver_Modbus_WriteSingleCoil(id->valueint, START_MOTOR_ADDR, isStart->valueint)
 *         
 *         // 4b、如果是 "get"（查询模式）：
 *            - 调用 cJSON_CreateObject() 创建返回 JSON 对象
 *            - 调用 cJSON_AddNumberToObject 添加 "id" 字段
 *            - 读取当前速度：
 *              调用 Driver_Modbus_ReadInputRegisters(id->valueint, CURRENT_SPPED_ADDR, 2, fx.arr)
 *              调用 cJSON_AddNumberToObject 添加 "currentSpeed" 字段
 *            - 读取当前角度：
 *              调用 Driver_Modbus_ReadInputRegisters(id->valueint, CURRENT_ANGLE_ADDR, 2, fx.arr)
 *              调用 cJSON_AddNumberToObject 添加 "currentAngle" 字段
 *            - 读取电机转向：
 *              定义 uint8_t dir = 0
 *              调用 Driver_Modbus_ReadDiscRegister(id->valueint, MOTOR_DIR_ADDR, &dir)
 *              调用 cJSON_AddStringToObject 添加 "dir" 字段（dir==1 ? "正转" : "反转"）
 *            - 调用 cJSON_PrintUnformatted(obj) 将 JSON 转为字符串
 *            - 调用 Common_Buffer_Write(upBuffer, json, strlen(json)) 写入上行缓冲
